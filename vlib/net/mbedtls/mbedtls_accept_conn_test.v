// vtest build: !(windows && tinyc) // TODO: fix these by adding declarations for the missing functions in the prebuilt tcc
module main

import net
import net.mbedtls
import time

// mbedtls_accept_conn_test.v — the SERVER-role handshake over an ALREADY
// ACCEPTED TCP connection (cx-private#1421).
//
// This module had two of the three combinations of {client, server} x {own
// socket, existing socket}: SSLConn.dial/connect (client) and
// SSLListener.accept (server, on a socket the listener itself bound). The
// third — server role over a socket that was accepted in the clear and has
// already carried cleartext traffic — is what an explicit upgrade (SMTP
// STARTTLS, RFC 3207; IMAP STARTTLS, RFC 9051 6.2.1) needs, and it is what
// new_ssl_server_conn + SSLConn.accept_conn add.
//
// The test is the upgrade in full: a plaintext line crosses the socket in
// BOTH directions first, so what is measured is genuinely a handshake over a
// used connection and not a TLS connection that merely skipped its listener.

const accept_conn_test_cert = '-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n'
const accept_conn_test_key = '-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----\n'

fn test_server_role_handshake_over_an_accepted_connection() {
	spawn fn () {
		time.sleep(60 * time.second)
		eprintln('mbedtls accept_conn test: timeout reached, exiting.')
		exit(1)
	}()

	mut listener := net.listen_tcp(.ip, '127.0.0.1:0') or {
		eprintln('SKIP: cannot bind a loopback listener: ${err}')
		return
	}
	port := listener.addr()!.port()!
	listener.set_accept_timeout(20 * time.second)

	server := spawn fn (mut l net.TcpListener) !string {
		defer {
			l.close() or {}
		}
		mut plain := l.accept()!
		// cleartext first, in both directions: this is the STARTTLS shape.
		mut buf := []u8{len: 64}
		n := plain.read(mut buf)!
		assert buf[..n].bytestr() == 'STARTTLS
'
		plain.write_string('220 go ahead
')!

		mut ssl := mbedtls.new_ssl_server_conn(
			cert:                   accept_conn_test_cert
			cert_key:               accept_conn_test_key
			validate:               false
			in_memory_verification: true
		)!
		ssl.accept_conn(mut plain)!
		mut sbuf := []u8{len: 64}
		sn := ssl.read(mut sbuf)!
		secret := sbuf[..sn].bytestr()
		ssl.write_string('pong-over-tls')!
		ssl.shutdown() or {}
		plain.close() or {}
		return secret
	}(mut listener)

	mut client := mbedtls.new_ssl_conn(validate: false)!
	mut connected := false
	for _ in 0 .. 40 {
		mut tcp := net.dial_tcp('127.0.0.1:${port}') or {
			time.sleep(50 * time.millisecond)
			continue
		}
		tcp.write_string('STARTTLS
')!
		mut greet := []u8{len: 64}
		gn := tcp.read(mut greet)!
		assert greet[..gn].bytestr() == '220 go ahead
'
		client.connect(mut tcp, 'localhost')!
		connected = true
		break
	}
	assert connected, 'the client never completed the upgrade handshake'

	client.write_string('ping-over-tls')!
	mut rbuf := []u8{len: 64}
	rn := client.read(mut rbuf)!
	assert rbuf[..rn].bytestr() == 'pong-over-tls'
	client.shutdown() or {}

	got := server.wait() or { panic('server leg failed: ${err}') }
	assert got == 'ping-over-tls'
}

// A server-role conn with no identity is refused where the operator can read
// it, not deep inside mbedtls with a numeric code.
fn test_server_conn_without_an_identity_is_refused() {
	if _ := mbedtls.new_ssl_server_conn(validate: false) {
		assert false, 'a server-role conn with no cert/key must not be constructible'
	} else {
		assert err.msg().contains('no certificate or key provided')
	}
}

// accept_conn on a CLIENT-role conn is a caller bug and says so.
fn test_accept_conn_refuses_a_client_role_conn() {
	mut listener := net.listen_tcp(.ip, '127.0.0.1:0') or {
		eprintln('SKIP: cannot bind a loopback listener: ${err}')
		return
	}
	port := listener.addr() or { panic(err) }.port() or { panic(err) }
	defer {
		listener.close() or {}
	}
	mut tcp := net.dial_tcp('127.0.0.1:${port}') or {
		eprintln('SKIP: cannot dial the loopback listener: ${err}')
		return
	}
	defer {
		tcp.close() or {}
	}
	mut client := mbedtls.new_ssl_conn(validate: false) or { panic(err) }
	if _ := client.accept_conn(mut tcp) {
		assert false, 'accept_conn must refuse a client-role SSLConn'
	} else {
		assert err.msg().contains('client-role')
	}
}
