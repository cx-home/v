		...
		ast.HashStmt {
			...
			match node.kind {
				'include', 'preinclude', 'postinclude' {
					...
					// in both zstd.o and cxstore.o). Emit a definition-include only in the
					...
					if is_def_include && (g.pref.use_cache || g.pref.build_mode == .build_module) && !owned_here {
						sb.writeln('')
						sb.writeln('// -usecache: skipped C definition-include "' + inc_target + '" from cached module `${node.mod}` (compiled once into its own object)')
					} else {
...
fn (mut g Gen) write_init_function() {
	...
	if g.nr_closures > 0 || (g.pref.use_cache && g.table.used_features.anon_fn) {
		...
		// cxstore getter) dereferences a garbage captured context → segfault. Gate on
...
fn (mut g Gen) interface_table() string {
	...
	for isym in interfaces {
		...
		if g.pref.skip_unused && !g.pref.use_cache
			&& isym.idx !in g.table.used_features.used_syms {
			continue
		}
...
return ${cast_shared_struct_str};
				...
				if shared_interface_mtx_helper_needed {
					// Compare against the index *symbol* via an `if`, not a `switch`
					// `case`: a case label needs an integer constant expression, but
					// under -usecache the index is a real `const u32` object (see the
					// def below) — usable in a runtime comparison but not as a constant.
					// This mirrors the `clone_cases` if-chain and keeps the dispatch
					// value globally identical across every TU (the program TU defines
					// the symbol; cached modules read it `extern`), instead of baking a
					// per-TU discovery-order literal that could diverge.
			...
			if g.pref.build_mode == .build_module {
				...
				// Under -usecache the owning module (e.g. builtin, which owns IError)
				// is compiled separately and its interface cast wrappers reference this
				// index as `extern const u32`. An `enum` member is not a linker symbol,
				// so the program TU must emit a real file-scope definition (external
				// linkage in C) so the cached objects resolve at link time. Every use
				// of the index is now a runtime read (cast wrappers, name_table subscript,
				// the shared-mtx if-chain above), so a `const u32` object suffices and the
				// value stays globally identical across the program TU and cached modules.
