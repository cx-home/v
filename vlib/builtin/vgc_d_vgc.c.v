...
// ─────────────────────────────────────────────────────────────────────────────
// #63 PASSIVE CAPTURE INSTRUMENT — recovered from session 0feb7822 (reference) +
// reviewed. Reproduces the GOLD victim correlation with NO masking. Includes ONLY:
//   (1) UAF detector (vgc_uaf_check_buf/_report/_classify),
//   (2) swept-log written DIRECTLY by the sweep free-path (vgc_slog_record),
//   (3) GOLD correlation (read-after-free vs swept-log → 0xc0de/0xc0d1).
// EXCLUDES (non-negotiable, these masked the crash or were dead paths in cx_watch):
//   arming/ring, quarantine/poison, holder-find/reverse-scan, forced collections.
// Function bodies are always compiled but only CALLED from `$if vgc_passive ?`
// sites in map.v/string.v/the sweep loop, so the default build is unaffected.
// Tags: 0xbf1=freed buf ptr, 0xbf2=len, 0xbf3=size-class; 0xc0de=GOLD victim,
//       0xc0d1=freed-at-gen; 0xda1-3/0xdb0/0xdc0=absurd-len report+classify.
// ─────────────────────────────────────────────────────────────────────────────
