...
static inline void vgc_park_spill(uint32_t* stop_flag, uint32_t* stop_seq,
    ...
    // mutator behind the sweep-while-live UAF lineage.
...
// ============================================================
  ...
  // Retained as a fallback. This is the path that exhibited the macOS-only
...
#else
  // ---- macOS SIGNAL-based STW (DEFAULT) -----------------------------------------
  // Mirrors the Linux signal-suspend that is PROVEN sound for multi-reactor http:serve
  // under -gc e churn (A/B: macOS mach-suspend exhibits the UAF; the Linux
  // signal-suspend at 20x the allocation throughput is clean). The async mach_suspend + external thread_get_state
  ...
  // Forensic helper: is `val` present in ANY currently-parked thread's captured
  ...
  // Forensic helper: is the thread with mach-port `t` currently parked in the
  ...
  static void vgc_suspend_handler(int sig, siginfo_t* si, void* uctx) {
      ...
      // mark/sweep for CPU (a near-idle server showed ~37% of a sleeping
      // worker's samples inside this loop, and parked reactors slowed the
  ...
  // the STW-completeness accounting counted it as covered. That is exactly the
  // sweep-while-live shape. A live registered
  ...
  static inline int vgc_suspend_thread(uint32_t t) {
      ...
      // born unmarked -> swept while live). Retry
  ...
  // Forensic twin of the darwin helper: search parked threads' captured regs.
  static inline int vgc_captured_regs_contain(uintptr_t val) {
  ...
  static void vgc_suspend_handler(int sig, siginfo_t* si, void* uctx) {
      ...
      // parked thread and competing with the collector for CPU (see
  ...
  // was a soundness hole (sweep-while-live residual);
  ...
  static inline int vgc_suspend_thread(uint32_t t) {
      ...
      // (see the darwin twin — dropping a live peer = the sweep-while-live UAF).
      {
...
// name the runtime type. Async-signal-safe / non-allocating (no GC re-entry).
static inline void vgc_verify_report(uint64_t kind, uint64_t referrer_addr, uint64_t referrer_size,
    ...
    // struct layout can be matched to a V type by eye. Capped at 8 words.
    vgc__ws(" words=[");
